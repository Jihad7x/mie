...
build: deps ## Build the mie binary
	@echo "Building mie $(VERSION) for $(GOOS)/$(GOARCH)..."
	@mkdir -p bin
	CGO_ENABLED=1 CGO_LDFLAGS="$(CGO_LDFLAGS)" go build -tags cozodb $(LDFLAGS) -o bin/mie ./cmd/mie
	@echo "Built bin/mie"

install: build ## Install mie binary globally (default: ~/go/bin, override: INSTALL_DIR=path)
	@mkdir -p $(INSTALL_DIR)
	@cp bin/mie $(INSTALL_DIR)/mie
	@echo "Installed mie to $(INSTALL_DIR)/mie"
...
test: deps ## Run all tests with race detection and coverage
	@echo "Running tests..."
	CGO_ENABLED=1 CGO_LDFLAGS="$(CGO_LDFLAGS)" go test -tags cozodb -race -cover -coverprofile=coverage.out ./...
	@echo "Coverage report: coverage.out"

test-short: ## Run tests without integration tests (no CozoDB required)
	@echo "Running short tests..."
	CGO_ENABLED=1 CGO_LDFLAGS="$(CGO_LDFLAGS)" go test -tags cozodb -short -race ./...
...
run: ## Run the application
	go run -tags cozodb ./cmd/mie $(ARGS)

help: ## Show help
	@echo "MIE - Memory Intelligence Engine"
	@echo ""
