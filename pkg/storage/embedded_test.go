...
func TestNewEmbeddedBackend_DefaultDataDir(t *testing.T) {
	config := EmbeddedConfig{
		Engine: "mem",
		// DataDir not specified - should default to ~/.mie/data
	}
	backend, err := NewEmbeddedBackend(config)
	if err != nil {
...
func TestNewEmbeddedBackend_ProjectID(t *testing.T) {
	config := EmbeddedConfig{
		...
		// DataDir not specified - should use ~/.mie/data/test-project
	}
	backend, err := NewEmbeddedBackend(config)
	if err != nil {
...
func TestEmbeddedBackend_EnsureSchema(t *testing.T) {
	...
	// Verify mie_meta table was created by querying it
	ctx := context.Background()
	result, err := backend.Query(ctx, "?[key, value] := *mie_meta{key, value} :limit 1")
	if err != nil {
...
// TestEmbeddedBackend_CreateHNSWIndex tests HNSW index creation placeholder.
func TestEmbeddedBackend_CreateHNSWIndex(t *testing.T) {
	...
	// CreateHNSWIndex is a placeholder that should succeed without error
...
// TestEmbeddedBackend_Meta tests the metadata storage.
func TestEmbeddedBackend_Meta(t *testing.T) {
	backend := setupTestStorage(t)
	defer func() {
		_ = backend.Close()
	}()
	...
	// Test GetMeta with non-existent key returns empty string
	value, err := backend.GetMeta("nonexistent")
	if err != nil {
	...
	// Test SetMeta
	err = backend.SetMeta("test_key", "test_value")
	if err != nil {
		t.Fatalf("SetMeta failed: %v", err)
	}

	// Test GetMeta retrieves the value
	value, err = backend.GetMeta("test_key")
	if err != nil {
		t.Fatalf("GetMeta failed: %v", err)
	}
	if value != "test_value" {
	...
	err = backend.SetMeta("test_key", "new_value")
	if err != nil {
	...
	value, err = backend.GetMeta("test_key")
	if err != nil {
		t.Fatalf("GetMeta after overwrite failed: %v", err)
	}
	if value != "new_value" {
